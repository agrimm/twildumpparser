class ProcessingSharedLibrary
  ...
  def self.create_repository_sql_file(filename)
    raise "File already there" if File.exist?(filename)
    file = File.open(filename, "w")
    initial_text = "insert into `repositories` (id, abbreviation, short_description, created_at, updated_at) VALUES ;"
    file.write(initial_text)
    file.close
  end

  #Return an array containing all of the repositories listed
  def self.list_repositories(file)
...
#    raise "No match found between #{repository_text} and #{record_re}" unless repository_text =~ record_re
  ...
  def self.find_repository(filename, language_code)
...
#        puts "id match is #{id_match}"
  ...
  def self.find_unused_repository_id(file)
    repositories = self.list_repositories(file)
  ...
  def self.add_repository(filename, language_code)
    #Check that it isn't already in there
    raise "Already exists" unless self.find_repository(filename, language_code).nil?
    if File.exists?(filename)
      repository_id = self.find_unused_repository_id(File.open(filename))
    else
      repository_id = 1
    end
    raise "Language code not found" if LanguageCode.language_code[language_code].nil?
    abbreviation = language_code + "wiki"
    short_description = LanguageCode.language_code[language_code] + " language Wikipedia"
    created_at = "now()"
    updated_at = "now()"
    if File.exists?(filename)
  ...
  def self.find_or_add_repository(filename, language_code)
    repository_id = self.find_repository(filename, language_code)
    if repository_id.nil?
      repository_id = self.add_repository(filename, language_code)
    end
    return repository_id
  end
